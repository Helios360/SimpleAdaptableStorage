<script lang="ts">
  let cogOpen = $state(false);

  function toggleTheme() {
    const root = document.documentElement;
    const cur = getComputedStyle(root).getPropertyValue('--primary').trim();
    const goingDark = cur === '#dff6f6';
    root.style.setProperty('--primary',   goingDark ? '#141414' : '#dff6f6');
    root.style.setProperty('--secondary', goingDark ? '#dff6f6' : '#141414');
    localStorage.setItem('dark', String(goingDark));
  }
</script>

<header>
  <a href="/" aria-label="Accueil">
    <img class="logo" src="/sources/LogoBleuOmbre-edited.png" alt="Logo" />
  </a>

  <div class="settings">
    <button
      type="button"
      aria-label="Paramètres"
      aria-expanded={cogOpen}
      onclick={() => (cogOpen = !cogOpen)}
    >
      <svg id="cog" class:open={cogOpen} fill="var(--secondary)" width="60" height="60" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
        <path d="M18.1 11c-3.9 0-7 3.1-7 7s3.1 7 7 7 7-3.1 7-7-3.1-7-7-7m0 12c-2.8 0-5-2.2-5-5s2.2-5 5-5 5 2.2 5 5-2.2 5-5 5"/>
        <path d="m32.8 14.7-2.8-.9-.6-1.5 1.4-2.6c.3-.6.2-1.4-.3-1.9l-2.4-2.4c-.5-.5-1.3-.6-1.9-.3l-2.6 1.4-1.5-.6-.9-2.8C21 2.5 20.4 2 19.7 2h-3.4c-.7 0-1.3.5-1.4 1.2L14 6c-.6.1-1.1.3-1.6.6L9.8 5.2c-.6-.3-1.4-.2-1.9.3L5.5 7.9c-.5.5-.6 1.3-.3 1.9l1.3 2.5c-.2.5-.4 1.1-.6 1.6l-2.8.9c-.6.2-1.1.8-1.1 1.5v3.4c0 .7.5 1.3 1.2 1.5l2.8.9.6 1.5-1.4 2.6c-.3.6-.2 1.4.3 1.9l2.4 2.4c.5.5 1.3.6 1.9.3l2.6-1.4 1.5.6.9 2.9c.2.6.8 1.1 1.5 1.1h3.4c.7 0 1.3-.5 1.5-1.1l.9-2.9 1.5-.6 2.6 1.4c.6.3 1.4.2 1.9-.3l2.4-2.4c.5-.5.6-1.3.3-1.9l-1.4-2.6.6-1.5 2.9-.9c.6-.2 1.1-.8 1.1-1.5v-3.4c0-.7-.5-1.4-1.2-1.6m-.8 4.7-3.6 1.1-.1.5-.9 2.1-.3.5 1.8 3.3-2 2-3.3-1.8-.5.3q-1.05.6-2.1.9l-.5.1-1.1 3.6h-2.8l-1.1-3.6-.5-.1-2.1-.9-.5-.3-3.3 1.8-2-2 1.8-3.3-.3-.5Q8 22.05 7.7 21l-.1-.5L4 19.4v-2.8l3.4-1 .2-.5c.2-.8.5-1.5.9-2.2l.3-.5-1.7-3.3 2-2 3.2 1.8.5-.3c.7-.4 1.4-.7 2.2-.9l.5-.2L16.6 4h2.8l1.1 3.5.5.2q1.05.3 2.1.9l.5.3 3.3-1.8 2 2-1.8 3.3.3.5q.6 1.05.9 2.1l.1.5 3.6 1.1z"/>
        <path fill="none" d="M0 0h36v36H0z"/>
      </svg>
    </button>

    <button type="button" class="setting" class:open={cogOpen} aria-label="Changer de thème" onclick={toggleTheme}>
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M13.589 21.659c-3.873 1.038-8.517-.545-10.98-3.632a1 1 0 0 1 .751-1.623c3.984-.118 6.662-1.485 8.17-4.098 1.51-2.613 1.354-5.616-.535-9.125a1 1 0 0 1 1.03-1.463c3.904.59 7.597 3.82 8.635 7.694 1.43 5.334-1.737 10.818-7.071 12.247" fill="var(--secondary)"/>
      </svg>
    </button>
  </div>
</header>
